...
# task keys, none the dashboard alone read. Both print request, priority and issue: the task skeleton as
# placeholders task-new.sh refuses until they are filled, the block skeleton as values task-new.sh replaces with
# its parent's.
set -u
...
fill() { # <kind> <goal> > filled markdown
  sh "$bin/task-template.sh" "$1" | sed '
    ...
    s/<R-YYYYMMDD-n|null>/R-20260924-1/; s/<P0|P1|P2|P3>/P1/; s#<forge issue url|null>#https://forge.example/i/1#' \
...
check "2b the block takes request, priority and issue from its parent" \
  'request: R-20260924-1|priority: P1|issue: https://forge.example/i/1|' \
  "$(grep -E '^(request|priority|issue):' "$bf" 2>/dev/null | tr '\n' '|')"
...
# --- 2e: the three lines of the org: request, priority, issue ------------------------
check "2e the task skeleton prints the three lines as placeholders" \
  'request: <R-YYYYMMDD-n|null>|priority: <P0|P1|P2|P3>|issue: <forge issue url|null>|' \
  "$(sh "$bin/task-template.sh" task | grep -E '^(request|priority|issue):' | tr '\n' '|')"
check "2e the block skeleton prints the three lines with the defaults" 'request: null|priority: P2|issue: null|' \
  "$(sh "$bin/task-template.sh" block | grep -E '^(request|priority|issue):' | tr '\n' '|')"
sh "$bin/task-template.sh" task | sed 's/<key>/demo/; s/<green|yellow|red>/green/; s/<feature|bugfix|refactor|research|ops>/bugfix/
  s/<low|medium|high>/low/; s/<YYYY-MM-DD>/2026-09-24/' \
  | awk '/^# Goal$/ { print; print "fix(demo): unfilled"; skip = 1; next } skip && /^$/ { skip = 0 } !skip' > "$tmp/raw.md"
rc=0
out=$(sh "$bin/task-new.sh" --repo demo --state "$state" --file "$tmp/raw.md" 2>&1) || rc=$?
check "2e task-new.sh refuses the three placeholders unfilled" 1 "$rc"
