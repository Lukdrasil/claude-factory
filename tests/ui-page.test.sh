...
# more sessions and an ask of every kind, served through ui-up.sh and driven by tests/ui-page.test.js in a
# Playwright container on the host network, as the host uid so the asks it writes mid-run belong to the host.
...
timeout 15m docker run --rm --network host --user "$(id -u):$(id -g)" -e HOME=/tmp \
  ...
  -v "$ui:$ui" -v "$repo/tests/ui-page.test.js:/t/ui-page.test.js:ro" "$pw" node /t/ui-page.test.js > "$tmp/page.out" 2>&1
rc=$?
cat "$tmp/page.out"
is 'the browser checks exit 0'                                "$rc" 0
has 'the browser checks ran'                                  '^PASS |^FAIL ' "$(cat "$tmp/page.out")"
