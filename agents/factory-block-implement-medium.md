...
description: Implements one high complexity block inside a factory solve session, given the block task and (for a yellow/red block) the ## Handoff from factory-block-tests. Spawned by the coordinator per _shared/delegation.md; do not call it directly.
model: opus
effort: medium
