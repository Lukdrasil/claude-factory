...
description: Reads and judges the recon a triage investigation gathered — the inventory, the find-refs hits, the worker-explorer answers — and writes the five-section ## Investigation report. Read-only, spawns nothing. Spawned per _shared/investigate.md; do not call it directly.
model: opus
effort: high
...
`find-refs` hits over the entry point and the answers from up to 8 `worker-explorer` recon calls. Your job is
