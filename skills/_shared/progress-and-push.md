...
transition, commits in the state clone and pushes (ADR-0050). There is no dashboard, so never send the user to
one: a task with an MR ends in `done` and a triage, ops or research task ends in `closed`, both through
