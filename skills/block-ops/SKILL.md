---
name: block-ops
description: Carries out the small forge action a task describes: a comment, a label, creating, closing or reopening an issue via glab/gh. No product clone, no code, no MR; the output is the action plus evidence in the progress file.
---

# block-ops

Carry out **exactly the forge action the task describes**, nothing more. An ops task has no product clone and
no `branch:` (ADR-0018); the state clone is `$WORK_DIR/state` and the only tools for the action are `glab`
and `gh`.
...
3. Carry out the action, `gh issue comment <url> --body "..."` or `glab issue note <url> -m "..."`, label,
   close and reopen alike; the host in the URL tells you the forge. `${CLAUDE_PLUGIN_ROOT}/bin/forge.sh hosts`
   lists the signed-in instances, and `${CLAUDE_PLUGIN_ROOT}/bin/forge.sh issue <url>` reads the issue first
...
Change **only** `status:` in your own task's frontmatter; `owner` is set by your claim, `attempt` and
`plan_hash` by `task-approve.sh`, `ready` and `closed` by the human (P5).

| status | when |
|---|---|
| `review` | the action is done and the evidence is under `## Evidence`; a human verifies it and closes it with `task-done.sh` |
...
repo (ADR-0047), so status and snapshot go through
