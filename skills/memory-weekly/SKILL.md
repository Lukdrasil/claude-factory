...
1. **Read** every `drafts/*.md`, every `memory/proposals/*.md` with a `Replaces:` line (the daily pass leaves
   them here), `playbook.md` (`wc -w`) and the open `k1/*.md`. No draft and no such proposal: stamp (step 6)
   and report. Completion: the drafts are listed with their lesson counts, the proposals with what they
   replace.
2. **Judge.** Spawn the `memory-curator` agent with a brief naming the scope, the draft paths, the
   ...
   promote, and its word count. No draft: skip it. Completion: every draft has a verdict and the word count
   is at most 800.
3. **Rounds.** Through `_shared/ask.md` (flow `memory`, step `weekly <scope>`), at most five items per
   round, one question per item. A draft: its title, its lessons, the curator's text or edit, and the options
   promote, k1, wait, drop with the curator's verdict as the recommendation. A `Replaces:` proposal: what it
   replaces (every path) and why (its text), and the options yes or no, nothing recommended. An open K1 draft
   is a notice in the first round, and one the human calls merged is deleted in step 4. Completion: every
   item of the round has an answer.
4. **Apply the round.**
