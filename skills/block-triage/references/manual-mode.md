...
The worker procedure in `SKILL.md` applies with these deltas:

- **No triage task.** Skip steps 1 and 8, so no progress snapshot and no self-report; the run ends when the draft task
  is finished. In `## Internal` omit the `- triage:` line: there is no triage task for the auto-close to pair with.
