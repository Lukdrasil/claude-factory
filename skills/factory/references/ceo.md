...
`factory`, herdr agent name `ceo`. The workspace also holds the `factory-ui-relay` tab and every pre-approval
...
herd-watch.sh prints `<unit> <status> <phase> <agent> <mr>`, with agent `working`, `blocked`, `ready`,
`gone`, `closed` or `unknown`, and `<unit> waits <what>`:

| line | what you do |
|---|---|
| a step unit `<T-id>-<step>` reads `ready` or `gone` | `sh <plugin-root>/bin/solve-next.sh <T-id>`; when the state shows the step's output (Chain), dispatch the next step |
| `<unit> waits <what>` | a step waits on the human; herd-watch already ran `notify.sh`. One terminal line: which tab, the UI url. Never answer for the human |
| agent `blocked` | a dialog. `herdr agent read <name> --source recent-unwrapped --lines 120`, ask the human (`_shared/ask.md`), answer with `herdr agent send-keys <name> <keys>`, never `herdr agent prompt`. With `ui: docker` the human may answer in the pane instead: name it |
| agent `gone` with no output in the state | dispatch the step again; two deaths in a row is `_shared/blocked-question.md` |
| a lead unit `<T-id>-lead` reads `gone` or `closed` | the lead ended: its parent is done, or it waits on a cross-repo parent; `--queue` restarts it when it may |
| a parent reads `done` | when it was the last parent of its request: `sh <plugin-root>/bin/map.sh status <R-id> done` |
...
- Related issues: after triage, offer to link one of them (`issue: <url>` in the parent's frontmatter,
  ...
  carries `ai-drafted`).
- Chart: one chart session per request, `--step chart` on the first parent whose triage is done; its prompt is
  `/claude-factory:wayfinder chart <R-id> <T-id>`. One map covers every repository of the request, and
  research tickets run in it as subagents. The other parents wait for `map.sh clear`. You always try to chart
  (W3); a map with no fog continues straight into the plain grill.
- Grilling rounds are the human's: the map's tickets and the grill rounds reach the tab and the UI through
  `_shared/ask.md`. Their state transitions (`grilling`, `planned`) are `map.sh status` calls of the session
  that caused them.
...
1. `sh <plugin-root>/bin/map.sh ticket <R-id> grilling "<the need>" --repo <key>` and `sh
   <plugin-root>/bin/map.sh status <R-id> grilling`. The human answers it like any grilling ticket: in scope or
   out of scope.
...
3. Out of scope: `sh <plugin-root>/bin/map.sh drop <R-id> <NN>` with its line under Out of scope; ask the human
   whether it becomes a new request, and tell the lead.

## Memory

`pass-stamp.sh --due daily|weekly` lists the scopes whose pass is due (`global`, `repo:<key>`, `agent:<a>`,
`repo-agent:<key>/<a>`); the Memory tab shows the same dates and its start button posts `start the <daily|weekly>
pass for <scope>` into this session. Nothing starts without the human's go, in the terminal or through that
button.

- Daily: an interactive step session in its own herdr workspace, `sh <plugin-root>/bin/session-monitor.sh
  --step pass --scope <key>/<agent>` (unit `pass-<key>-<agent>`, counted under `sessions`), which runs
  `claude-factory:memory-daily <scope>` and stamps the pass itself. Watch it like a step.
