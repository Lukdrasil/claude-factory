...
this session, not as a spawned research task. Result: proposals with `Replaces:` in the scope's `proposals/`
queue, then curated per `curation:` (`references/curate.md`): the `memory-curator` agent when `auto`, the
human walk when `manual`. Approving one deletes every file its `Replaces:` names in the same commit
(`curate-apply.sh`), so nobody removes the sources by hand.

## When

On demand only: the user asks to consolidate, or `factory doctor` names an over-budget scope
(`missing: memory over budget in <scope>, run factory consolidate <scope>`, `<plugin-root>/bin/memory-budget.sh`).
Never scheduled: consolidation is not part of `factory solve` or any hook.
...
The repo x agent tier (`repos/<key>/agents/<agent>/memory/`) is not consolidated here: the daily and weekly
passes (`memory-daily`, `memory-weekly`) distill it into the agent's playbook.
...
- **Report.** Per scope: files read, proposals written, and (once curated) the counts by decision and
  `memory-budget.sh <scope>` before and after.
